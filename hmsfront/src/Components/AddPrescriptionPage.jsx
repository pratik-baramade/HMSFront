...
const AddPrescriptionPage = () => {
  ...
  // Fetch medicine and symptom data on page load
  useEffect(() => {
    // Fetch pharmacy records (medicines)
    fetch("http://localhost:8081/myapp/pharmacy")
      .then((res) => res.json())
      .then((data) => {
  ...
  // Function to get price for selected medicine
  const getMedicinePrice = (selectedName) => {
    const medicine = medicineList.find((med) => med.name === selectedName);
  ...
  const handleChange = (e) => {
    ...
    if (name === "medicine") {
      setFormData((prev) => ({
        ...
        medicine: value,
        customMedicine: "",
        price: value === "other" ? "" : getMedicinePrice(value)
      }));
    } else {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              readOnly={formData.medicine !== "other"}
              className={`w-full border border-gray-300 rounded px-3 py-2 ${
                formData.medicine !== "other" ? "bg-gray-100 cursor-not-allowed" : ""
              }`}
            />
          </div>
        </div>
